...
export function DashboardScreen() {
  ...
  const metrics = useMemo(() => {
    ...
    const topRating = products.reduce((acc, product) => Math.max(acc, product.rating), 0);
  ...
  const recentProducts = useMemo(() => [...products].sort((a, b) => b.id - a.id).slice(0, 6), [products]);
  ...
  const productsByCategory = useMemo(() => {
    ...
    const othersTotal = sortedCategories.slice(TOP_CATEGORY_COUNT).reduce((acc, item) => acc + item.total, 0);
      ...
      <section aria-label="Key metrics" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Card role="article" aria-label="Average product price">
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl tabular-nums">{currencyFormatter.format(metrics.averagePrice)}</CardTitle>
          </CardHeader>
        </Card>
            ...
            <figure aria-labelledby="stock-by-brand-title" aria-describedby="stock-by-brand-summary">
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stockByBrandWithColors} margin={{ top: 12, right: 4, left: 0, bottom: 0 }}>
                    ...
                    />
                    <Bar dataKey="stock" radius={[6, 6, 0, 0]} fill="var(--chart-1)" stroke="var(--card)" strokeWidth={1}>
                      {stockByBrandWithColors.map((entry) => (
                        <Cell key={entry.brand} fill={entry.color} stroke="var(--card)" strokeWidth={1} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ...
              <figcaption id="stock-by-brand-summary" className="sr-only">
                {stockByBrandWithColors.length
                  ? `Stock by brand: ${stockByBrandWithColors.map((entry) => `${entry.brand} ${entry.stock}`).join(", ")}.`
        ...
        <Card>
          ...
          <CardContent>
            ...
            <ul className="mt-3 grid gap-1 text-sm sm:grid-cols-2" aria-label="Products by category legend">
              {productsByCategoryWithColors.map((entry) => (
                <li key={`${entry.category}-legend`} className="flex items-center gap-2">
                  <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: entry.color }} aria-hidden="true" />
                  <span>
                ...
                <TableBody>
                  {recentProducts.map((product) => (
                    <TableRow key={product.id} className="hover:bg-muted/60">
                      ...
                      <TableCell className="text-right tabular-nums">{currencyFormatter.format(product.price)}</TableCell>
                      <TableCell className="text-right tabular-nums">{product.stock}</TableCell>
                      <TableCell className="text-right tabular-nums">{product.rating.toFixed(1)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
